...
<head>
    ...
    <script type="text/javascript" src="/scripts/business/fund/fund-new.js"></script>
    <title>销量管理平台</title>
</head>
<body ng-app="channelApp">
<div class="pd-20" ng-controller="fund-new">
    <div class="">
    ...
    <table class="table table-border table-bordered table-hover table-bg table-sort">
        ...
        <tr class="text-c" ng-repeat="p in items">
            ...
            <td>{{p.code}}</td>
            <td>{{p.fundName}}</td>
            <td>{{p.fundCompanyName}}</td>
            <td>{{p.fundCompanyCode }}</td>
            <td>{{p.period }}</td>
            <td class="td-manage">
                <c:if test="${null != operate && operate.edit == 1}">
                    <a title="停用" href="javascript:;" ng-click="update(p)" class="ml-5" style="text-decoration:none">
                        <i class="Hui-iconfont">&#xe631;</i>
                    </a>
                </c:if>
                <c:if test="${null != operate && operate.delete == 1}">
                    <a title="公司" target="_blank" href='http://fund.eastmoney.com/company/{{p.fundCompanyCode}}.html'
                       class="ml-5"
                       style="text-decoration:none"><i class="Hui-iconfont">&#xe643;</i>
                    </a>
                </c:if>
                <c:if test="${null != operate && operate.delete == 1}">
                    <a title="基金" target="_blank"
                       href='http://fund.eastmoney.com/{{p.code}}.html?spm=search'
                       class="ml-5"
                       style="text-decoration:none"><i class="Hui-iconfont">&#xe666;</i>
                    </a>
                </c:if>
