...
channelApp.controller('bill-good-edit', ['$scope', '$http', '$location', function ($scope, $http, $location) {
    $scope.result = null;
    $scope.form = null;

    $scope.edit = function () {
        $http.post('/bill-good/edit', angular.toJson($scope.form))
    ...
    $scope.find = function () {
        ...
        $http.get('/bill-good/find?id=' + id.value)
            .success(function (data) {
                $scope.form = data;
